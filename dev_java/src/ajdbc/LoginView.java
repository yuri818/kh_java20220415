...
public class LoginView extends JFrame implements ActionListener{
	...
	JTextField  jtf_id		 = new JTextField("apple",10);
	JTextField  jtf_pw		 = new JTextField("123",10);
...
//		this.pack(); //알아서 가로세로 크기 정해줌
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == jbtn_login) {
			...
			LoginDao ld = new LoginDao(); //이제 더이상 initDisplay()호출 안됨
			//사용자가 입력한 아이디와 비번을 Dao클래스의 login메소드에 파라미터로 넘김
			result = ld.login(user_id, user_pw);
